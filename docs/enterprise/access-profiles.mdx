...
allowed it. The entity's per-provider and per-model limits are charged when its profile allows that
provider and model. A customer reached through both a team and a business unit is charged once; the
...
explains what changes, lets you name the new profile and choose the providers the entity's keys may
reach, previews exactly what will move, then runs the migration:

- A new access profile is created for this entity alone, from its current budgets and rate limit.
- Its virtual keys can reach the providers you chose: **all providers**, **specific providers** (all of
  their models), or **no providers** - every request is refused until you add providers to the profile.
- Every key the entity owns is adopted; budgets, rate limits and providers set on the keys are removed.
- Spend is kept: the profile's budgets use the legacy budgets' reset windows, so everything already
  spent counts against them, on every node of a cluster.
- The legacy budgets and rate limit are deleted.
...
curl -X POST "$BIFROST/api/governance/teams/$TEAM_ID/access-profile/migrate" \
  ...
  -d '{"name": "Platform access", "provider_access": "selected", "providers": ["openai"], "dry_run": true}'
```
...
A key created under an entity that holds a profile, or moved under one, is adopted the same way: any
budgets, rate limits, or provider configs in the request are stripped, just as for keys created by a
user who holds a profile.
